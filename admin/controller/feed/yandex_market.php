...
class ControllerFeedYandexMarket extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			...
			$this->redirect((HTTPS_SERVER . 'index.php?token=' . $this->session->data['token'] . '&route=extension/feed'));
		}
		...
		$this->document->breadcrumbs[] = array(
			'href'      => (HTTPS_SERVER . 'index.php?token=' . $this->session->data['token'] . '&route=extension/feed'),
			'text'      => $this->language->get('text_feed'),
			'separator' => ' :: '
		);

		$this->document->breadcrumbs[] = array(
			'href'      => (HTTPS_SERVER . 'index.php?token=' . $this->session->data['token'] . '&route=feed/yml'),
			'text'      => $this->language->get('heading_title'),
			'separator' => ' :: '
		);

		$this->data['action'] = (HTTPS_SERVER . 'index.php?token=' . $this->session->data['token'] . '&route=feed/yandex_market');

		$this->data['cancel'] = (HTTPS_SERVER . 'index.php?token=' . $this->session->data['token'] . '&route=extension/feed');
