	...
	public function index() {
		...
		foreach ($languages as $language) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => HTTPS_SERVER . 'index.php?route=module/htmlmodule1&token=' . $this->session->data['token'],
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);

		$this->data['action'] = HTTPS_SERVER . 'index.php?route=module/htmlmodule1&token=' . $this->session->data['token'];
		...
		if (isset($this->request->post[$this->_name . '_code'])) {
			$this->data[$this->_name . 'htmlmodule1_code'] = $this->request->post[$this->_name . '_code'];
		} else {
